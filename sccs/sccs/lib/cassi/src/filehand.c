 ...
 *
 * @(#)filehand.c	1.8 20/09/06 J. Schilling
 */
#if defined(sun)
#pragma ident "@(#)filehand.c 1.8 20/09/06 J. Schilling"
#endif
/*
...
static int ftrans	__PR((int opcode, char *file1, char *file2, char *area, int asize));
static int copyrest	__PR((FILE *fp1, FILE *fp2, char *place, int bsize));
static int getrec	__PR((FILE *file, char *buffer, int recsep, int rectype, int maxlen));
static int rec_cmp	__PR((char **args, char **fmatch, int recsep));
static int argchop	__PR((char *s, int fldsep, char **sargs));
...
int	asize;				/* Of buffer area. */
{
	...
	retcode = copyrest (fp1, fp2, area, asize);
	if (fclose (fp1) == ERROR)
		retcode = LIVESRC;
	if (fclose (fp2) == ERROR)
		retcode = NOCLOSE;
...
static int
copyrest (fp1, fp2, place, bsize)		/* Copy the rest of a file. */
...
int	bsize;					/* Size of buffer. */
{
	...
	if (place == EMPTY) {
		if (bsize <= 0)
			bsize = BUFSIZE;
		if ((space = (char *) malloc ((unsigned) bsize)) == EMPTY) {
	...
	while ((count = fread (space, sizeof (char), (unsigned) bsize, fp1)) >
	  (unsigned) 0)
...
begin:
	...
	while ((i = getrec (fp1, realrec, recsep, rectype, maxlen)) == DONE) {
		...
		if (chop == (int (*) __PR((char *, int, char **))) NULL)
		...
		if (comparef == (int (*) __PR((char **, char **, int))) NULL)
			cmpstat = rec_cmp (arg, fmatch, recsep);
		else
			cmpstat = (*comparef) (arg, fmatch, recsep);
