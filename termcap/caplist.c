/* @(#)caplist.c	1.10 12/04/27 Copyright 2000-2012 J. Schilling */
/*
...
{ "cs",	"csr",		"change_scroll_region",		"change scrolling region to line #1 to line #2 (NP)" },
...
{ "CM",	"mrcup",	"cursor_mem_address",		"memory relative cursor addressing to row #1 column #2 (N)" },
...
{ "te",	"rmcup",	"exit_ca_mode",			"strings to end programs using cursor_address" },
...
{ "vb",	"flash",	"flash_screen",			"visible bell (must not move cursor)" },
...
{ "if",	"if",		"init_file",			"name of file containing initialization string" },
...
{ "r1",	"rs1",		"reset_1string",		"reset string (reset terminal completely)" },
{ "r2",	"rs2",		"reset_2string",		"reset string (reset terminal completely)" },
{ "r3",	"rs3",		"reset_3string",		"reset string (reset terminal completely)" },
...
{ "wi",	"wind",		"set_window",			"set current window to lines #1-#2 cols #3-#4 (N)" },
...
{ "rs",	"rs",		"termcap_reset",		"terminal reset string (reset terminal completely)" },
	 ...
	 */
{ "rP",	"rmp",		"char_padding",			"like insert_padding but when in replace mode" },
...
{ "eA",	"enacs",	"ena_acs",			"enable alternate (graphic) char set" },
...
{ "&9",	"kBEG",		"key_sbeg",			"shifted begin key" },
{ "&0",	"kCAN",		"key_scancel",			"shifted cancel key" },
{ "*1",	"kCMD",		"key_scommand",			"shifted command key" },
{ "*2",	"kCPY",		"key_scopy",			"shifted copy key" },
{ "*3",	"kCRT",		"key_screate",			"shifted create key" },
{ "*4",	"kDC",		"key_sdc",			"shifted delete-char key" },
{ "*5",	"kDL",		"key_sdl",			"shifted delete-line key" },
{ "*6",	"kslt",		"key_select",			"select key" },
{ "*7",	"kEND",		"key_send",			"shifted end key" },
{ "*8",	"kEOL",		"key_seol",			"shifted clear-to-end-of-line key" },
{ "*9",	"kEXT",		"key_sexit",			"shifted exit key" },
{ "*0",	"kFND",		"key_sfind",			"shifted find key" },
{ "#1",	"kHLP",		"key_shelp",			"shifted help key" },
{ "#2",	"kHOM",		"key_shome",			"shifted home key" },
{ "#3",	"kIC",		"key_sic",			"shifted input key" },
{ "#4",	"kLFT",		"key_sleft",			"shifted left-arrow key" },
{ "%a",	"kMSG",		"key_smessage",			"shifted message key" },
{ "%b",	"kMOV",		"key_smove",			"shifted move key" },
{ "%c",	"kNXT",		"key_snext",			"shifted next key" },
{ "%d",	"kOPT",		"key_soptions",			"shifted options key" },
{ "%e",	"kPRV",		"key_sprevious",		"shifted previous key" },
{ "%f",	"kPRT",		"key_sprint",			"shifted print key" },
{ "%g",	"kRDO",		"key_sredo",			"shifted redo key" },
{ "%h",	"kRPL",		"key_sreplace",			"shifted replace key" },
{ "%i",	"kRIT",		"key_sright",			"shifted right-arrow key" },
{ "%j",	"kRES",		"key_srsume",			"shifted resume key" },
{ "!1",	"kSAV",		"key_ssave",			"shifted save key" },
{ "!2",	"kSPD",		"key_ssuspend",			"shifted suspend key" },
{ "!3",	"kUND",		"key_sundo",			"shifted undo key" },
...
{ "xr",	"xr",		"return_does_clr_eol",		"return clears to end of line" },
...
{ "--",	"--",		"OBSOLETE",			"--" },
{ "--",	"--",		"INT",				"--" },
{ "dB",	"dB",		"backspace_delay",		"ms of padding required for ^H" },
{ "dF",	"dF",		"form_feed_delay",		"ms of padding required for ^L" },
{ "dT",	"dT",		"horizontal_tab_delay",		"ms of padding required for ^I" },
{ "dV",	"dV",		"vertical_tab_delay",		"ms of padding required for ^K" },
{ "kn",	"kn",		"number_of_function_keys",	"count of function keys" },
{ "dC",	"dC",		"carriage_return_delay",	"ms of padding required for CR" },
{ "dN",	"dN",		"new_line_delay",		"ms of padding required for LF" },
